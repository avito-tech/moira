...
var (
	...
	emojiStates          = map[string]string{
		moira.OK:     "\xe2\x9c\x85",
		moira.WARN:   "\xe2\x9a\xa0",
		moira.ERROR:  "\xe2\xad\x95",
		moira.NODATA: "\xf0\x9f\x92\xa3",
		moira.TEST:   "\xf0\x9f\x98\x8a",
	}
)
...
func (sender *Sender) Init(senderSettings map[string]string, location *time.Location) error {
	...
	sender.logger = logging.GetLogger("")
...
func (sender *Sender) Loop(timeout time.Duration) {
	...
	for {
		message, ok := <-messages
		if !ok {
			sender.logger.Warn("Telegram messages channel was closed, stop listening and deregister")
			sender.DataBase.DeregisterBot(messenger)
			return
		}
		if err := sender.handleMessage(message); err != nil {
			sender.logger.ErrorF("Error sending message: %v", err)
		}
	}
}
...
func (sender *Sender) renewSubscription(ttl time.Duration) {
	...
	for {
		...
		if !sender.DataBase.RenewBotRegistration(messenger) {
			sender.logger.WarnF("Could not renew subscription for %s bot, try to register bot again", messenger)
			if !sender.DataBase.RegisterBotIfAlreadyNot(messenger, ttl) {
