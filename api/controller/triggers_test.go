...
func TestCreateTrigger(t *testing.T) {
	...
	Convey("Success with trigger.ID empty", t, func() {
		...
		resp, err := CreateTrigger(dataBase, nil, &triggerModel, make(map[string]bool))
		So(err, ShouldBeNil)
	...
	Convey("Success with triggerID", t, func() {
		...
		dataBase.EXPECT().GetTrigger(triggerModel.ID).Return(nil, database.ErrNil)
		dataBase.EXPECT().AcquireTriggerCheckLock(gomock.Any())
		dataBase.EXPECT().DeleteTriggerCheckLock(gomock.Any())
		dataBase.EXPECT().GetOrCreateTriggerLastCheck(gomock.Any()).Return(&moira.CheckData{
			Metrics: make(map[string]*moira.MetricState),
	...
	Convey("Get trigger error", t, func() {
		...
		resp, err := CreateTrigger(dataBase, nil, &trigger, make(map[string]bool))
...
func TestGetAllTriggers(t *testing.T) {
	...
	Convey("Has triggers", t, func() {
		...
		database.EXPECT().GetTriggerIDs(false).Return(triggerIDs, nil)
	...
	Convey("No triggers", t, func() {
		database.EXPECT().GetTriggerIDs(false).Return(make([]string, 0), nil)
	...
	Convey("GetTriggerIDs error", t, func() {
		...
		database.EXPECT().GetTriggerIDs(false).Return(nil, expected)
		list, err := GetAllTriggers(database)
		So(err, ShouldResemble, api.ErrorInternalServer(expected))
		So(list, ShouldBeNil)
	})
